...
{
    internal sealed class DoctorAuthenticationService: IDoctorAuthenticationService
    {
        ...
        public DoctorAuthenticationService(IRepositoryManager repository, ILoggerManager logger, IMapper mapper, UserManager<SystemUser> userManager, IOptions<JwtConfiguration> configuration)
        {
            _logger = logger;
            _mapper = mapper;
        ...
        public async Task<IdentityResult> RegisterUser(DoctorUserCreationDto doctorUserDto)
        {
            var user = _mapper.Map<SystemUser>(doctorUserDto);
            user.Role = "Doctor";
            var doctor = _mapper.Map<DoctorUser>(doctorUserDto);
            var result = await _userManager.CreateAsync(user, doctorUserDto.Password);
            if (result.Succeeded)
